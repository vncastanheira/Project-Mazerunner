...
public class VisibleFrame : MonoBehaviour
{
    ...
    [Tooltip("Image bilboard, rotate around own center")]
    public bool Bilboard;

    [Tooltip("Select visible layers to check against")]
    ...
    void Start()
    {
        ...
        {
            visibleRenderer = visibleFrame.GetComponent<SpriteRenderer>();
    ...
    void Update()
    {
        ...
        Vector3 thisP = transform.position;
        int layerMask = VisibleLayer.value;
        ...
        if (bestFrame != null)
        {
            var bestRenderer = bestFrame.GetComponent<SpriteRenderer>();
            visibleRenderer.sprite = bestRenderer.sprite;
            visibleRenderer.flipX = bestRenderer.flipX;
            visibleRenderer.flipY = bestRenderer.flipY;

            if (Bilboard)
            {
                //var upwards = visibleFrame.transform.position.y;
                //var target = Camera.main.transform.position;
                //target.y = upwards;
                //visibleFrame.transform.LookAt(target);
                visibleFrame.transform.LookAt(Camera.main.transform);
            }
            else
            {
